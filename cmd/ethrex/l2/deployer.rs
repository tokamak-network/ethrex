...
pub struct DeployerOptions {
    ...
    #[arg(
        ...
        default_value = "false",
        value_name = "BOOLEAN",
        env = "ETHREX_L2_TOKAMAK",
        action = ArgAction::Set,
        help_heading = "Deployer options",
        help = "If true, L2 will require Tokamak proofs to validate batch proofs and settle state."
    )]
    pub tokamak: bool,
    #[arg(
        long = "tokamak.verifier-address",
        value_name = "ADDRESS",
        env = "ETHREX_DEPLOYER_TOKAMAK_VERIFIER_ADDRESS",
        help_heading = "Deployer options",
        help = "Address of a pre-deployed Tokamak verifier contract"
    )]
    pub tokamak_verifier_address: Option<Address>,
    #[arg(
        long,
        default_value = "false",
        value_name = "BOOLEAN",
...
/// Bytecode of the TokamakVerifier contract.
/// Copied from tokamak-network/Tokamak-zk-EVM-contracts (main branch).
...
const INITIALIZE_ON_CHAIN_PROPOSER_SIGNATURE_BASED: &str = "initialize(bool,address,bool,bool,bool,bool,bool,address,address,address,address,address,bytes32,bytes32,bytes32,bytes32,address,uint256,address)";
const INITIALIZE_ON_CHAIN_PROPOSER_SIGNATURE: &str = "initialize(bool,address,bool,bool,bool,bool,bool,address,address,address,address,address,bytes32,bytes32,bytes32,bytes32,uint256,address,address)";
...
pub struct ContractAddresses {
    ...
    pub tokamak_verifier_address: Address,
...
async fn deploy_contracts(
    ...
    if opts.sp1 && opts.tokamak {
        return Err(DeployerError::InternalError(
            "SP1 and Tokamak are mutually exclusive proof backends. Please select only one."
                .to_string(),
        ));
    }
    ...
    // if it's a required proof type, but no address has been specified, deploy it.
    let (tokamak_verifier_deployment_tx_hash, tokamak_verifier_address) =
        match opts.tokamak_verifier_address {
            Some(addr) if opts.tokamak => (H256::default(), addr),
            None if opts.tokamak => {
                info!("Deploying TokamakVerifier");
                let (tx_hash, tokamak_verifier_address) =
                    create2_deploy_from_bytecode_no_wait(
                        &[],
                        TOKAMAK_VERIFIER_BYTECODE,
                        deployer,
                        &salt,
                        eth_client,
                        Overrides {
                    ...
                    .await?;
                info!(address = %format!("{tokamak_verifier_address:#x}"), tx_hash = %format!("{tx_hash:#x}"), "TokamakVerifier deployed");
                (tx_hash, tokamak_verifier_address)
            }
            _ => (H256::default(), Address::zero()),
        };
    ...
    trace!(
        ...
        tokamak_verifier_address = ?tokamak_verifier_address,
        "Contracts deployed"
    );
    let mut receipts = vec![
        ...
        tokamak_verifier_deployment_tx_hash,
        router_deployment.implementation_tx_hash,
        router_deployment.proxy_tx_hash,
    ];
...
async fn initialize_contracts(
    ...
    if opts.deploy_based_contracts {
        ...
        let calldata_values = vec![
            ...
            Value::Address(contract_addresses.tokamak_verifier_address),
...
fn write_contract_addresses_to_env(
    ...
    writeln!(
        writer,
        "ETHREX_DEPLOYER_TOKAMAK_VERIFIER_ADDRESS={:#x}",
        contract_addresses.tokamak_verifier_address
    )?;
