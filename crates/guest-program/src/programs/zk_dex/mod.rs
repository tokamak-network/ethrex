...
> {
    ...
    let initial_order_count =
        read_dex_storage(&dex_storage_trie, storage::orders_length_slot());
            ...
            if let Ok(sender) = tx.sender() {
                ...
                if to_addr == dex_contract {
                    ...
                    if sel == &transfer_sel && data.len() >= 4 + 96 {
                        ...
                        let order_index =
                            initial_order_count + U256::from(make_order_offset);
                        for field in 0..7u64 {
                            storage_slots.insert((
                                dex_contract,
                        ...
                        if !parent_note_u256.is_zero() {
                            let parent_note =
                                H256::from(parent_note_u256.to_big_endian());
                            storage_slots.insert((
                                dex_contract,
