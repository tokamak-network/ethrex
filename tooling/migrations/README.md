...
`--resume-from-checkpoint` derives resume start from a checkpoint file (`resume_start = checkpoint.target_head + 1`) and applies the same head validation. The checkpoint must match the current `schema_version`; unsupported schema versions are rejected explicitly. `--resume-from-block` and `--resume-from-checkpoint` are mutually exclusive.
