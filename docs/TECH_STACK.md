...
## 6. 구현 결정 레코드 (Decision Record)

### D-001: In-repo 구현 언어
- 기본 구현은 **Rust/Go 우선**(ethrex 레포 생태계/배포 파이프라인 정합성 우선)
- Python은 PoC/보조 도구로만 허용
- 최종 배포 아티팩트는 저장소 표준 CI를 통과해야 함

### D-002: 인터페이스/출력 표준
- 모든 커맨드는 사람이 읽는 로그 + `--json` 구조화 출력 동시 지원
- 에러 코드는 단계별로 고정하여 자동화에서 분기 가능하도록 함

### D-003: 영속 상태 저장
- 마이그레이션 체크포인트는 재실행 가능한 형식으로 저장
- 롤백/재개는 동일 상태 파일을 공유해 결정적 동작 보장
