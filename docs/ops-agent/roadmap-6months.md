...
| 월 | KPI 게이트 |
|---|---|
| Month 1 | end-to-end 파이프라인 성공률 95%+, 감사 로그 누락 0 |
| Month 2 | incident 중복 생성률 10% 이하, 탐지 지연 p95 < 30s |
| Month 3 | 저위험 자동조치 성공률 85%+, rollback/escalation 누락 0 |
| Month 4 | 무승인 high 조치 0건, 승인 리드타임 p95 < 5분 |
| Month 5 | 오탐 조치율 5% 이하, 재발 incident MTTR 20% 개선 |
| Month 6 | 준자율 운영 모드에서 주간 안정성 SLO 충족 |

## 5. LLM 사용 경계 (안전 규칙)

- LLM은 원인 요약/설명/권고에만 사용 (read-only)
- 실행 권한 결정은 규칙 엔진 + 승인 체계가 최종권한
- 민감정보/비밀값은 프롬프트 입력 금지
- LLM 제안은 감사 로그에 근거와 함께 저장
