...
### P0 (필수)
- T-001 ~ T-004: 스키마 분석/매핑/변환/검증 핵심 경로 완성
- Dry-run 모드 + 리포트 포맷 고정(JSON 스키마 포함)
- 롤백 드릴 자동 테스트 1회 이상

### P1 (바로 다음)
- 에러 재시도 정책 고정(일시/치명 분류)
- 체크포인트 재개(resume) 및 운영 로그 개선

### 완료 정의 (Definition of Done)
- 기능 구현 + 테스트 + 실행 예시 문서 + 실패 시나리오 증빙까지 포함
- PR에는 “입력/출력/롤백 영향” 3항목 체크리스트 필수
